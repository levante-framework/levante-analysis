...
| `10_downward_extension.qmd` | **Downward extension, first tranche (Boston Children's, ages 2–5; RAW data, no validated scoring).** Pulls `pilot_bostonchildrens_us_main_raw:7e6c` v4_26 (31 children, 228 runs, 9 tasks) via `levante::get_raw_table()`. Hand-scores each task from trial-level correctness (H&F 200–2000 ms RT recode mirroring `recode_trials()`, vocab catch vs test, Corsi max span, Math by subtask, chance from `distractors`) and describes accuracy / time-in-task / trials-completed by age. Findings: strong age gradients on H&F, TROG, Vocab, Memory, SDS; **2-year-olds at chance on nearly everything and failing vocab catch trials** (engagement gate); Math has no floor below 4; CAT θ pinned at the −6 estimator floor for the youngest. Hand scores reproduce the dashboard counts exactly where comparable. **Not in the public book render list on purpose** (unreleased raw child data). |
